...
contract SpaceBank {
    ...
    function deposit(
        uint256 amount,
        bytes calldata data
    ) external _emergencyAlarms(data) {
        require(
    ...
    function flashLoan(uint256 amount, address flashLoanReceiver) external {
        ...
        (bool success, bytes memory result) = flashLoanReceiver.call(
        ...
        require(
            currentBalance >= initialBalance + fee,
    ...
    function _emergencyAlarmProtocol(bytes memory data) internal {
        ...
        if (EmergencyAlarms == 2) {
            ...
            require(
                ...
                "You need to send ether to pass through security"
            );
